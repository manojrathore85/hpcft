...
sub fill_users
	...
	'while not rs.eof
	%>
	<option value="<%=arrData(0,i)%>"><%=arrData(1,i) & " " & arrData(2,i)%></option>
	<%
	'rs.movenext
	'wend
	next
	%>
	<option value="All_Summarized">All_Summarized</option>
	<option value="All_PerUser">All_PerUser</option>
	<%
'end if
...
function get_create_count(between_expr)
	...
	if request("lstUsers") = "All_Summarized" then
		sql = "select count(*) from issues where createdate " & between_expr
	else
		sql = "select count(*) from issues where createdate " & between_expr & " and reporter = '" & the_user & "'"
	end if
...
function get_update_count(between_expr)
	...
	if request("lstUsers") = "All_Summarized" then
		sql = "select count(*) from issuechangescomments where updatedate " & between_expr & " and field <> 'deleted'"
	else
		sql = "select count(*) from issuechangescomments where updatedate " & between_expr & " and changesby = '" & the_user & "' and field <> 'deleted'"
	end if
...
sub create_report
	...
	case "daily"
		...
		get_data_for_report count_of,"d"
	case "weekly"
		count_of = datediff("ww",start_date,end_date,vbMonday)
		get_data_for_report count_of,"ww"
	case "monthly"
		count_of = datediff("m",start_date,end_date)
		get_data_for_report count_of,"m"
	case "yearly"
		count_of = datediff("yyyy",start_date,end_date)
		get_data_for_report count_of,"yyyy"
	end select
end sub

sub get_data_for_report(count_of,vb_add_interval_type)
	dim i
	dim date_from 
	dim date_to
	dim between_expr
...
sub report_for_all_users
	dim i
	
	For i = iRecFirst To iRecLast
	'while not rs.eof
		the_user = arrData(0,i)
	%>
    <tr bgcolor="#FFFFFF"><td colspan="4" align="center"><strong><%=arrData(1,i) & " " & arrData(2,i)%></strong></td></tr>
	<!--<option value="<%=arrData(0,i)%>"><%=arrData(1,i) & " " & arrData(2,i)%></option>-->
	<%
	call create_report
	'rs.movenext
	...
	Erase arrData ' destructing array object
...
<tr>
  <td><table width="100%" border="0" cellpadding="1" cellspacing="1"><form name="frmStats" method="post" action="">
		...
		if request("submit") = "Generate Report" then
			...
			the_user = request("lstUsers")
			
			if request("lstUsers") = "All_PerUser" then
				call report_for_all_users
			else
				'the_user = request("lstUsers")
				call create_report
...
%>
frmStats.lst_start_year.value = '<%=year(date)%>';
frmStats.lst_start_month.value = <%=month(date) - 1%>; 
frmStats.lst_start_day.value = <%=day(date)%>;
frmStats.lst_end_year.value = '<%=year(date)%>';
frmStats.lst_end_month.value = <%=month(date) - 1%>; 
frmStats.lst_end_day.value = <%=day(date)%>;
<%
end if
%>
