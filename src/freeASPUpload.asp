...
Class FreeASPUpload
	...
	Public Sub Save(path)
		...
		For Each fileItem In UploadedFiles.Items
			fileItem.Filename = check_filename(path,fileItem.filename)
	...
	' by atul20120330
	' function takes path and filename as input parameters
	' checks whether file is available in the path 
	' if yes it produces a new name 
	' if no returns the same file name
	Public Function check_filename(path,filename)
'	on error goto err1
		dim boolValidExt
		boolValidExt = false
		dim i, counter, datestr
		counter = 0
set fso = server.CreateObject("Scripting.FileSystemObject")
		for i=lbound(validExtensions) to ubound(validExtensions)
			if mid(filename,instr(1,filename,".") + 1,len(filename)) = validExtensions(i) then
				boolValidExt = true
				exit for
			end if
		next

		if boolValidExt = true then
			datestr = year(date)
			if len(month(date)) = 1 then
				datestr = datestr & "0" & month(date)
			else
				datestr = datestr & month(date)
			end if
			if len(day(date)) = 1 then
				datestr = datestr & "0" & day(date)
			else
				datestr = datestr & day(date)
			end if
			if len(hour(now)) = 1 then
				datestr = datestr & "0" & hour(now)
			else
				datestr = datestr & hour(now)
			end if
			if len(minute(now)) = 1 then
				datestr = datestr & "0" & minute(now)
			else
				datestr = datestr & minute(now)
			end if
			if len(second(now)) = 1 then
				datestr = datestr & "0" & second(now)
			else
				datestr = datestr & second(now)
			end if
			...
			do
				if fso.fileExists(path & "\" & filename)then
					tempStr = mid(filename,1,instr(1,filename,".") -1) & datestr 
					'if i = 1 then
					'	tempStr = mid(file.filename,1,instr(1,file.filename,".") -1) & datestr 
					'else
					'	tempStr = mid(file.filename,1,instr(1,file.filename,".") -1) & datestr & i
					'end if	
					filename = tempStr & "." & mid(filename,instr(1,filename,".") + 1,len(filename))
				else
					exit do
				end if
				i = i + 1
			...
			response.Write("File uploaded is not of the extensions allowed ( " & join(validExtensions,",") & " )")
			response.End()
		end if
